...
In this section, we'll fix an integer $b\in\mathbb{Z}$ and a set

$$
B = \{|\phi_1\rangle,\dots,|\phi_b\rangle\} \subset L^2(\mathbb{R}^3)
$$

of $b$ linearly independent vectors in $L^2(\mathbb{R}^3)$.
...
into a matrix equation involving the coordinates $\mathbf{C}$.
Substituting \ref{eq:linear-expansion}
into \ref{eq:fock-equation}:
...
The _Fock matrix_ is defined in terms of the
...
The _overlap matrix_ records the inner product of each pair of basis vectors.
...
We can now rewrite the Fock equation \ref{eq:fock-matrix-equation}
...
> matrix and let $\mathbf{F}(\mathbf{P},\mathcal{M})\in\mathrm{Mat}\_{b\times b}(\mathbb{C})$
> be the associated Fock matrix.
>
