---
layout: post
title: "Hartree Fock I: Ground State Estimation"
date: 2025-07-26
mathjax: true
utterance-issue: 13
---
...
![water molecule](/assets/scf/h2o.png)
...
to derive a system of $N$ quadratic equations relating the positions of the balls 
to the energy of the system. To solve for the state of the system at rest, 
we can minimize the energy by setting the derivative of the energy to zero and solving
the resulting system of $N$ linear equations with $N$ variables.
This method can be extended to arbitrary classical systems using the
[finite element method](https://en.wikipedia.org/wiki/Finite_element_method).

Extending this method to molecular simulation requires an efficient method for evaluating
and minimizing the energy of a molecular state.
...
Here is a density plot of one of the hydrogen atom's electron orbitals:

![orbital-3d](/assets/scf/orbital-3d.png)

Efficiently encoding an electron orbital in computer memory already poses an interesting challenge.
...
when considering how the state space scales with the number of electrons.

In classical mechanics, the state of $n$ particles is determined by a list of length $n$
containing the state of each particle.
...
to each possible length-$n$ list.
...
is determined by assigning a probability amplitude to each possible length $n$ bit-string which
requires $2^n$ complex numbers.
...
All of the algorithms discussed in this series are implemented from scratch in a
library called [slaterform](https://github.com/lowdanie/slaterform).

The repo's [readme](https://github.com/lowdanie/slaterform)
contains sample code for estimating a water molecule's electronic ground state and energy.
Here is a density plot of the electrons in the resulting state:

![water electron density](/assets/scf/h2o-density.png)
...
> $\psi_1,\dots,\psi_4\in L^2(\mathbb{R}^3)$ be single-electron wave functions.
>
> Then:
>
> $$
...
The double-electron inner-products required by the
[SCF algorithm](#the-scf-algorithm)
can be computed more efficiently by exploiting the following symmetry.

> **Claim (Electron Repulsion Symmetry).**
> Let $n\in\mathbb{Z}$ be a positive integer and let
> $\psi_1,\dots,\psi_4\in L^2(\mathbb{R}^3, \mathbb{R})$ be real-valued
> single-electron wave functions.
>
> Then the double-electron inner product is invariant to the following permutations:
>
> $$
> \begin{align*}
> \langle \psi_1\psi_2 | V_\mathrm{ee}^2 | \psi_3\psi_4 \rangle
> &= \langle \psi_3\psi_2 | V_\mathrm{ee}^2 | \psi_1\psi_4 \rangle \\
> &=\langle \psi_1\psi_4 | V_\mathrm{ee}^2 | \psi_3\psi_2 \rangle \\
> &= \langle \psi_2\psi_1 | V_\mathrm{ee}^2 | \psi_4\psi_3 \rangle
> \end{align*}
> $$

This implies that to evaluate the double-electron inner product for all 
$24$ possible orderings of $\psi_1,\dots,\psi_4$,
only $24/2^3 = 3$ of them must be computed.
...
$H^n(\mathbf{r};\mathcal{M})\in\mathrm{End}(\Lambda^n\mathcal{H})$ as a matrix. 

One idea could be to discretize $\mathbb{R}$ into a finite set of
points and express a positional wave-function $\psi\in L^2(\mathbb{R}^3)$ 
...
$L^2(\mathbb{R}^3)$ is $100^3$ dimensional and so
...
states that for any state $\|v\rangle\in V$ with norm $1$,
...
In our context, consider a molecule with nuclear state
...
are orthonormal. Note that by [Closed Shell Norm](#clm:closed-shell-norm),
$\|\Psi\rangle$ has unit length.
...
> $\mathcal{M}\in\mathbb{R}^{3\times m}\times\mathbb{Z}^m$
> a nuclear state and $n\in\mathbb{Z}$ an even integer.
>
...
$\psi_1,\dots,\psi_{n/2}\in L^2(\mathbb{R}^3)$ that minimize
...
we'll introduce Lagrange multipliers $\lambda_{kl}\in\mathbb{R}$
...
for $(\psi_1,\dots,\psi_{n/2})\in L^2(\mathbb{R}^3)^{n/2}$
to be a critical point of $L$.
...
as the trace the Coulomb potential with respect to a given density.
...
Intuitively, we can think of the trace as integrating the kernel $A$ over $V$.
...
on $W$. When $V=W$, we'll denote the partial trace on $\mathrm{End}(V\otimes V)$ with respect
...
> Let $V$ be a Hilbert space, $A$ an operator on $V$ and $|v\rangle\in V$ a vector.
>
> Then:
>
> $$
...
> Let $V$ be a Hilbert space,
> $n\in\mathbb{Z}$ an integer, 
> $A\in\mathrm{End}(V)$ an operator on $V$,
> $\|v_1\rangle,\dots,\|v_n\rangle\in V$
> orthonormal vectors and
> $\rho\in\mathrm{End}(V)$ the corresponding density operator
>
> Then
>
> $$
...
> $n\in\mathbb{Z}$ be an integer,
> $A\in\mathrm{End}(V\otimes W)$ an operator on $V\otimes W$,
> $\|w_1\rangle,\dots,\|w_n\rangle\in W$
> orthonormal vectors and
> $\rho_W\in\mathrm{End}(W)$ the corresponding density operator.
>
> Then
>
> $$
...
>
> Let $U\in\mathrm{End}(V)$ be a unitary transformation that maps the span
> of the $\|v_i\rangle$ to itself.
>
...
> Let $n\in\mathbb{Z}$ be an even integer and
> $\|\psi_1\rangle,\dots,\|\psi_{n/2}\rangle\in L^2(\mathbb{R}^3)$
> orthonormal positional wave-functions.
>
...
each positional wave-function $\|\psi_i\rangle$ represents two electrons: one 
...
In other words, if
$(\psi_1,\dots,\psi_{n/2})\in L^2(\mathbb{R}^3)^{n/2}$
...
By the definition of $L$ and linearity of the first variation,
...
> Let $n\in\mathbb{Z}$ be an even integer,
> $\|\psi_1\rangle,\dots,|\psi_{n/2}\rangle\in L^2(\mathbb{R}^3)$
> orthonormal single-electron states and
...
>
> $$
> \delta E(\psi_1,\dots,\psi_{n/2}; \mathcal{M}) =
> 4\mathrm{Re}\left( 
...
$$
E_1(\psi_1,\dots,\psi_{n/2}) =
2\sum_{j=1}^{n/2}\langle \psi_j | H^1(\mathbf{r};\mathcal{M}) | \psi_j \rangle
$$
...
\delta E_1 = 4\mathrm{Re}\left(
   \sum_{j=1}^{n/2}\langle \delta\psi_i | H^1(\mathbf{r};\mathcal{M}) | \psi_i \rangle
\right)
$$
...
By [Electron Repulsion Expectation](#clm:electron-repulsion-expectation):
...
> Let $m\in\mathbb{Z}$ be a positive integer,
> $\mathcal{M}\in\mathbb{R}^{3\times m}\times\mathbb{Z}^m$
> a nuclear state and $\rho\in\mathrm{End}(L^2(\mathbb{R}^3))$ a density operator.
>
...
eigenvectors $\|\psi'_i\rangle$ is equal to the original density $\rho$.
Therefore:
...
solve for eigenvectors of $F(\mathbf{r}; \rho, \mathcal{M})$.
We'll instead apply an iterative approach which alternates between fixing $\rho$ 
and solving for the eigenvectors of $F(\mathbf{r}; \rho, \mathcal{M})$,
...
By [Closed Shell Expectation](#clm:closed-shell-expectation)
and [Electron Repulsion Expectation](#clm:electron-repulsion-expectation)
...
Recall that our goal is to find _orthonormal_ states in $L^2(\mathbb{R}^3)$
that are at a local minimum of the
[Hartree-Fock energy functional](#def:hartree-fock-energy-functional)

In this section is we'll derive a condition on the coefficients
...
$\mathbf{c}_i,\mathbf{c}_j\in\mathbb{C}^b$
...
> Let $n\in\mathbb{Z}$ be an even integer,
> $\mathbf{C}\in\mathrm{Mat}_{b\times n/2}(\mathbb{C})$
> be a coefficient matrix and for each $1\leq i \leq n/2$:
>
> $$
...
We can express $\rho$ in terms of the basis $B$ by substituting the linear expansion
\ref{eq:linear-expansion} into the equation above:
...
> states and $\mathbf{S}\in\mathrm{Mat}_{b\times b}(\mathbb{C})$ the associated
> overlap matrix.
>
...
> $$
>
> where the matrix $\mathbf{G}\in\mathrm{Mat}_{b\times b}(\mathbb{C})$ is
> defined by:
>
> $$
> G_{ij} =
...
determinant using the Fock and density matrices.
...
Substituting this into the Roothaan equation \ref{eq:roothan-fixed-p} gives us:
...
1. Compute the [single-electron matrix](#def:single-electron-matrix)
   $\mathbf{H}^1(\mathcal{M})\in\mathrm{Mat}\_{b\times b}(\mathbb{C})$:
...
1. Compute an inverse square root of $\mathbf{S}$ as follows. First diagonalize 
   ...
   The inverse square root, $\mathbf{X}\in\mathrm{Mat}\_{b\times b}(\mathbb{C})$,
   is obtained by:
...
The next step is to iteratively update the density $\mathbf{P}$.
...
1. Compute the eigen-decomposition of $\mathbf{X}^*\mathbf{F}\mathbf{X}$ and define
...
Here is the top level SCF loop from [slaterform](https://github.com/lowdanie/slaterform):

<div class="codeblock-with-filename">
<a href="https://github.com/lowdanie/slaterform/blob/main/slaterform/hartree_fock/scf.py">slaterform/hartree_fock/scf.py</a>
...
and estimates  the electronic ground state using a standard basis set from
...
print(f"Electronic Energy: {result.electronic_energy} Ha")
```
...
Total Energy: -74.96444760738025 Ha
Electronic Energy: -84.04881211726543 Ha
```

The converged value of `-74.96 Ha` is slightly higher than
[state of the art](https://www.sciencedirect.com/science/article/abs/pii/S0166128004002532)
estimates which are around `-76.06 Ha`. This is consistent with the variational principle
which implies that our estimate is an upper bound on the true ground state energy.

The electron density plot in the [introduction](#introduction) was generated by
evaluating the converged electron density on the points of a grid, saving it to a
[cube file](https://cubefile.readthedocs.io/en/latest/), and viewing the file
with [3dmol](https://3dmol.org/doc/index.html):
...
the inner products that are used to define the overlap matrix $\mathbf{S}$,
the single-electron matrix $\mathbf{H}^1$ and the double-electron matrix
$\mathbf{G}$. This will be the focus of the
[next post](/blog/2025/11/14/electron-integrals).
