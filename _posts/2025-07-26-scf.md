...
For example, consider the water molecule consists of one oxygen nucleus, 
...
the atomic numbers of the nuclei as input, and output an estimate for the bond 
lengths and angles. We'd also like the algorithm to be broadly applicable and include
...
can no longer be captured by a position and velocity. Rather, the state of a 
...
electron would require 4MB of memory. This is in contrast to 12 bytes to store the position of
a classical particle.

The difference in the memory footprint of a single electron is significant, but in terms of
asymptotic simulation time it is merely a constant. A more significant challenge arises
when considering the state of $N$ electrons.
...
a [superposition](https://en.wikipedia.org/wiki/Quantum_superposition) which is determined
by specifying the probability of being in each possible list.

To be concrete, suppose for simplicity that both the classical and quantum particles could
be in only one of two states: $0$ or $1$. 
The classical state of $N$ particles could then be described
by a length $N$ bit-string. On the other hand, the quantum state of $N$ particles
