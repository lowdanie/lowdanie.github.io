...
>
> Let $\mathrm{Id}^{n-k}\in \mathrm{End}(V^{\otimes (n-k)})$ be the identity.
>
...
when $T^k$ is symmetric, its symmetric extension $T^n$ may be obtained by applying
$T^k$ to each size $k$ subset of $V^{\otimes n}$ and taking the sum.
...
subsequences by $\binom{[n]}{k}\subset [n]^k$.
...
> Let $V$ be a vector space, $k \leq n\in\mathbb{Z}$ positive integers
> and $T^k\in\mathrm{End}(V^{\otimes k})$ a symmetric operator on $V^{\otimes k}$
> and let $T^n\in\mathrm{End}(V^{\otimes n})$ be its symmetric extension.
>
> For each increasing length $k$ subsequence $I\in\binom{[n]}{k}$,
> let $T_I^n\in\mathrm{End}(V^{\otimes n})$ be the transformation obtained by applying
> $T^k$ to the factors of $V^{\otimes n}$ indexed by $I$ and applying
...
Now, note that if $I\in\binom{[n]}{k}$ is an increasing subsequence and
...
Given $n$ vectors $\|v_1\rangle,\dots,\|v_n\rangle\in V$ be vectors and a sequence
