...
By the definition of the Slater determinant, it suffices to show that:
...
It's easy to see that if the vectors $|v_1\rangle,\dots,|v_n\rangle$ are linearly dependent
...
that $\mathrm{dim}\Lambda^n W = 1$. Therefore, there exists a scalar
$\Lambda^nT\in\mathbb{C}$ which satisfies:

$$
\mathrm{Alt}^n(T|v_1\rangle\otimes\dots\otimes T|v_n\rangle) =
\Lambda^nT \mathrm{Alt}^n(|v_1\rangle\otimes\dots\otimes |v_n\rangle)
$$

It remains to show that

$$
\mathrm{det}(T) = \Lambda^n T
$$

where $\mathrm{det}(T)$ is the standard matrix
[determinant](https://en.wikipedia.org/wiki/Determinant) of $T$.

The matrix determinant of $T\in\mathrm{End}(W)$ is
...
1. It maps the identity transformation to $1$: $\mathrm{det}(\mathrm{Id}) = 1$

1. It is multi-linear in each basis vector. Specifically, if $1 \leq i \leq n$ is an index,
   $\lambda\in\mathbb{R}$ is a scalar and
   $R,S\in\mathrm{End}(W)$ are two transformations such that 
   $T(\|v_i\rangle) = R(\|v_i\rangle) + S(\|v_i\rangle)$ and such that
   $T(\|v_i\rangle) = R(\|v_i\rangle) = S(\|v_i\rangle)$
   for all $j\neq i$ then:

   $$
   \mathrm{det}(T) = \mathrm{det}(R) + \mathrm{det}(S)
   $$
