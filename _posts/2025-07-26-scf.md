...
{: #clm:orthonormal-coordinates }
> **Definition (Orthonormal Coordinates).**
...
>
> $$
> \mathbf{C}^* \mathbf{C} S \mathbf{C} = \mathrm{Id}
> $$
...
$\mathbf{D}\in\mathrm{Diag}\_n(\mathbb{C})$. Following
[Orthonormal Coordinates](#clm:orthonormal-coordinates), we also want to satisfy the
...
and then solve the original equation \ref{eq:roothan-fixed-p} by setting:
