...
$$
\langle\psi_i\delta\psi_j | H^2 | \psi_i\psi_j\rangle =
\langle\delta\psi_j\delta\psi_i | H^2 | \psi_j\psi_i\rangle
$$

Together this we obtain:
...
by [Matrix Element First Variation](#clm:matrix-element-first-variation),
the first variation of the right-hand side of equation is given by:
...
By equation \ref{eq:fock-invariant-subspace}, the operator
$F(\mathbf{r}; \rho,\mathcal{M})$ restricts to an operator on $V$.

Since $F(\mathbf{r}; \rho,\mathcal{M})$ is Hermitian, there is a unitary transformation
$U\in\mathrm{End}(V)$ that diagonalizes $F(\mathbf{r}; \rho,\mathcal{M})$.
...
and let $\rho'$ be the their closed-shell density operator.

By [Anti-Symmetrization Transformation](#clm:anti-symmetrization-transformation),
...
and so the transformed basis is also a critical point of $L$.

Furthermore, both $\rho$ and $\rho'$ are projections on $V$ and so
...
a critical point of to $L$ consisting of eigenvectors of $F(\mathbf{r}; \rho, \mathcal{M})$.

The following claim formalizes the relationship between critical points of $L$ 
and eigenvectors of the Fock operator.
...
> transformation of $V$, they are eigenvectors of
> $F(\mathbf{r}; \rho, \mathcal{M})$.
...
$\|\psi_1\rangle,\dots,\|\psi_{n/2}\rangle$ are critical points of 
</div>
</details>
