...
construct mathematical proofs about their properties.
...
Note that a full correctness proof would include a proof that the output of
insertion sort is a permutation of the input. This permutation property was 
omitted in the interest of space and can be proved using the the same techniques
...
overview of the relationship between types and proofs.
...
Instances of types in the family can be built recursively via the `base` and `step` constructors.

For example, we can use the `base` constructor to build objects of type `LessThan n (n + 1)`:
...
defined to be _true_ if there exists an instance of that type and _false_ if not.
...
Numerals such as `2` are syntax sugar for the corresponding iterated application of the `Nat`
...
are identical by definition. We'll go onto more detail on tactics in the following sections.
...
> For all natural numbers `n : Nat`, `0` is less than `n + 1`.
...
To prove the proposition, it is sufficient to construct a function with this type.
...
`lean` also provides syntax sugar to construct lists using the common bracket notation.
...
[total](https://en.wikipedia.org/wiki/Connected_relation). Somewhat confusingly,
...
theorem IsLowerBound.trans {x y : α} {l : List α}  (hxy : x ≤ y) (hyl : IsLowerBound y l) :
  IsLowerBound x l := by
...
The `List α` type has two constructors that we must match:
...
We now have everything needed to prove that the output of `insertionSort` is sorted.
...
theorem insertSorted_lb {x y : α} {ys : List α} (h_lb : IsLowerBound x ys) (hxy : x ≤ y) :
  ...
  --           This follows from x ≤ z and the induction hypothesis.
  --        3. The claim follows by applying IsLowerBound.step to x ≤ z and 2.
...
theorem insertionSort_sorted (l : List α) : IsSorted (insertionSort l) := by
  ...
  -- The claim follow immediately from the insertSorted_sorted
