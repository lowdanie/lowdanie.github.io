...
In the next section we'll talk about the type of waveform we will be emitting and show how its reflections can be detected with an antenna. After that we'll analyze what happens when this type of wave is reflected off of a single object with a specified distance and velocity. Then we'll consider the more general situation in which we simultaneously receive reflections from multiple objects and present a simple algorithm that allows us to pick out individual objects. The algorithm will be based on the fast fourier transform which was discussed in LINK our previous post. Finally, we'll discuss some of the limitations of our simplified setup and mention some enhancements that are employed by real world systems.
...
for some constant $0 < k < 1$. As mentioned above, the factor $k$ accounts for the power that is lost during the trip. After demodulation, we get the signal:

$$
y(t) = kA(t - \frac{2R}{c})
$$

PICTURE of demodulated transmitted and received signal
...
$$
\begin{align}
\tilde{y}(t) &= kA(t - \frac{2R(t)}{c})e^{2\pi{i}\Omega(t - \frac{2R(t)}{c}} \\
             &= kA(t - \frac{2R_0}{c} + \frac{2vt}{c})e^{2\pi{i}\Omega(t - \frac{2R_0}{c} + \frac{vt}{c}}) \\
	     &\overset{(1)}{=} kA(t - \frac{2R_0}{c} + \frac{2vt}{c})e^{2\pi{i}(\Omega{t} - \frac{2R_0}{\lambda} + \frac{vt}{\lambda}}) \\
	     &\overset{(2)}{\simeq} kA(t - \frac{2R_0}{c})e^{2\pi{i}\Omega{t}}e^{-2\pi{i}\frac{2R_0}{\lambda}}e^{2\pi{i}\frac{vt}{\lambda}} \\
	     &\overset{(3)}{=} k'A(t - \frac{2R_0}{c})e^{2\pi{i}\Omega_D{t}}e^{2\pi{i}\Omega{t}}
\end{align}
$$

Equality $(1)$ comes from the identity $c = \frac{\lambda}{\Omega}$ where $\lambda$ being the wavelength of the carrier wave $e^{2\pi{i}\Omega t}$. $(2)$ follows from our assumption that $v \ll c$. Finally, equality $(3)$ is obtained by defining a new constant $k' := k e^{-2\pi{i}\frac{2R_0}{\lambda}}$ and by defining the _doppler frequency_ to be $\Omega_D := \frac{v}{\lambda}$.

After demodulation, we get the signal

$$
y(t) = k'A(t - \frac{2R_0}{c})e^{2\pi{i}\Omega_D{t}}
$$
...
Now that we know the distance we can try to estimate the velocity $v$. Recall that the doppler frequency was defined to be $\Omega_D = \frac{v}{\lambda}$ where $\lambda$ is the frequency of the carrier wave which we already know. So finding $v$ is equivalent to estimating $\Omega_D$. When we estimated the distance we only cared about finding a time $t_R$ for which $\|y(t_R)\|$ was not zero. In our search for $\Omega_D$ we'll consider the actual value of the signal $y(t)$ at $t=t_R$ which by the definition of the pulse $A(t)$ is

$$
y(t_R) = k'e^{2\pi{i}\Omega_D t_R}
$$

In effect, we've managed to obtain a sample of the function $k'e^{2\pi{i}\Omega_D t}$. Unfortunately, a single sample is not enough to determine the doppler frequency $\Omega_D$. However, recall from our previous post LINK that if we assume some bound on the magnitude of $\Omega_D$ (i.e the bandwidth) then by the sampling theorem we can recover $\Omega_D$ using multiple samples where the number of samples is determined by the bandwidth. Furthermore, we showed how this can be done efficiently using the fast Fourier transform.

The upshot of this discussion is that even though we cannot measure $\Omega_D$ using a single pulse, we may be able to do it with multiple pulses by using them to obtain many samples of $e^{2\pi{i}\Omega_D t}$ and then applying the FFT. This leads us to the next section in which we'll upgrade the single pulse to the  _pulse burst_ waveform and work through the details of using it to estimate $\Omega_D$.
...
$$
y(t) = \sum_{m=0}^{M-1}k A(t - mT - \frac{2R_0}{c})e^{2\pi{i}\Omega_D t}
$$

where $\Omega_D = \frac{v}{\lambda}$ and $k$ is a constant.
...
$$
\begin{align}
y[l, m] = y(mT + lT_s) &= \sum_{k=0}^{M-1}k A(mT + lT_s - kT - \frac{2R_0}{c})e^{2\pi{i}\Omega_D (mT + lT_s)} \\
                       &= k A(lT_s - \frac{2R_0}{c})e^{2\pi{i}\Omega_D (mT + lT_s)} \\
		       &= \begin{cases}
		            k e^{2\pi{i}\Omega_D (mT + lT_s)} & lT_s \simeq \frac{2R_0}{c} \\
			    0 & \mathrm{else}
			  \end{cases}
\end{align}
$$

In conclusion, all of the rows of $y[l, m]$ will be $0$ except for row $l=l_0$ satisfying $l_0 \simeq \frac{2R_0}{cT_s}$. This already allows us to estimate the distance by finding the non zero row $l_0$ and concluding that
...
$$
y[l_0, m] = k e^{2\pi{i}\Omega_D (mT + l_0T_s)} = k' e^{2\pi{i}\Omega_D mT}
$$

where we've absorbed the constant $e^{2\pi{i}\Omega_D l_0T_s}$ into the leading constant $k$. We can see that values in row $l_0$ consist of $M$ equally spaced samples of the function $f(t) = k' e^{2\pi{i}\Omega_D t}$ on the interval $[0, MT)$. Explicitly, we have sampled $f(t)$ at the points $t = 0, T_s, \dots, (M-1)T_s$ and got length $M$ vector $\mathbf{v} = [y[l_0, 0], \dots, y[l_0, M-1]]$. As we explained in the previous post LINK if we apply fast Fourier transform to $v$ then the result $\mathbf{w} = \mathrm{FFT(v)}$ will also be a length $M$ vector satisfying
...
and our estimate for the doppler frequency is $\Omega_D \simeq \frac{k_0}{MT}$. Since $\Omega_D = \frac{v}{\lambda}$, the velocity may be approximated by

$$
v \simeq k_0 \frac{\lambda}{MT}
$$
...
$$
z[l, k] = FFT(y[l, m]) = FFT(\sum_{i=0}^{N-1} y_i[l, m]) = \sum_{i=0}^{N-1} FFT(y_i[l, m]) = \sum_{i=0}^{N-1} z_i[l, k]
$$
...
$$
v_{\mathrm{max}} = \frac{(M-1)\lambda}{MT} \simeq \frac{\lambda}{T}
$$
...
When we receive a signal, we have no way of knowing whether it is from the beginning or end of the pulse. This ambiguity of $\tau$ seconds translates to an ambiguity of $c \tau$ meters in our range estimate. In other words, the resolution of our range estimate is $c \tau$ meters.
