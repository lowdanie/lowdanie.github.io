...
$\mathbf{x}\in (\mathbb{Z}/2)^N$ until we get the same result twice. Based on the analysis of
the
...
The _discrete logarithm_ of $x$ in base $g$ is defined to be the integer $0\leq s < p-1$
satisfying:
...
With this notation, we can say that the first step of the standard method uses
a single invocation  of $f$ to produce a coset state
...
and a function $f:G\rightarrow A$ that hides a subgroup $H\subset G$ of order $2$.
...
Note that if $\mathbf{y}=(y_1,\dots,y_N)$ then we can rewrite the righthand side of 
equation \ref{eq:bitstring-char} as:
...
$$
\mathbf{y}\cdot\mathbf{s} = 0\ (\mathrm{mod}\ 2)
$$
